...
fname =r"C:\Users\cg_h2\Documents\pucp_mini_array\c.txt" # file name to write output
id = "COM3" # com port name
...
while True:
    ...
    if counter == 0:
        ...
        file.write("### Event | Ardn_time[ms] | ADC_value[0-1023] | SiPM[mV] | Deadtime[ms] | Comp_UTC_time\n")
        file.write("###################################################################################\n")
    #file.write((str(data))[4:-2] + " " + str(time.time()) + "\n")

    decoded_data = data.decode('utf-8')[:-2] + ' '+ str(time.time()) + "\n"
